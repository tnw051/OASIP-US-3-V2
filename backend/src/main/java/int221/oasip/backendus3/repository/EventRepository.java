...
public interface EventRepository extends JpaRepository<Event, Integer> {
     ...
     * <p>{@code categoryIds} is optional. If it is not null, only events with the category ids will be returned.
     * <p>{@code userId} is optional. If it is not null, only events with the user id will be returned.
     *
     * @param startAt    start time of event
     * @param categoryId category id of event
    ...
    @Query("SELECT E FROM Event E " +
            "JOIN EventCategory EC ON E.eventCategory.id = EC.id " +
            "JOIN EventCategoryOwner ECO ON EC.id = ECO.eventCategory.id " +
            "JOIN User U ON ECO.user.id = U.id WHERE U.email = :email")
    List<Event> findAllByLecturerEmail(String email);

    List<Event> findByEventCategory_Id(Integer categoryIds);
}